...
def search1(search):
    conn = pymysql.connect(host='localhost', user='root', password='sql비밀번호작성',
    ...
    # SQL문 실행
    sql = "select title, location, scale, date_start, date_finish, size_one, size_two , price_one, price_two from main_subscription where location like '%{}%'".format(
        search)
    curs.execute(sql)
...
def foliumMap(month, size):
    ...
    tstt = pd.read_csv('./as_{}.csv'.format(month),
                       encoding='utf-8', engine="pyarrow")
    df = DataFrame()
    ...
    for i in range(len(sigungu_arr)):
        price_lst.append(tstt.query('시군구 == "{}"'.format(df.loc[i, '시군구'])).query(
            pyengsu[pyeng]).loc[:, '평단가'].mean())
    ...
    for i in range(df.count()[0]):
        position.append(poss.loc[poss['시군구'].str.contains(
            df.loc[i, '시군구'], case=False, na=False)].loc[:, ['위도', '경도']][-1:].values)

    print('비어있는 위치정보 삭제')
    df['위도', '경도'] = position
    index_num = []
    for i in range(df.count()[0]):
        index_ = poss.loc[poss['시군구'].str.contains(
            df.loc[i, '시군구'][:-1], case=False, na=False)].index
...
def model(locate, size):
    ...
    final_tst = pd.read_csv(
        './test_total.csv', encoding='utf-8', engine="pyarrow")
    ...
    for i in range(12, 22):
        for j in range(1, 13):
            ...
            price_mean = final_tst.query('시군구 == "{}"'.format(juso)).query(
                '계약년월=='+date).query(pyengsu[pyeng]).loc[:, '평단가'].mean()
    ...
    #    print("price_one : " ,price_one)
        ...
        model.add(keras.layers.LSTM(
            5, input_shape=(learn_num, 1), dropout=0.3))
        ...
        history = model.fit(train_input, train_target, epochs=100, validation_data=(
            val_input, val_target), callbacks=[checkpoint_cb, early_stopping_cb])

        if (len(history.history['loss']) > 20):
            pair_accuracy.append(['@@\n', learn_num, history.history['accuracy']
                                  [-3:-2][0], history.history['val_accuracy'][-3:-2][0]])
            pair_loss.append(['@@\n', learn_num, history.history['loss']
                              [-3:-2][0], history.history['val_loss'][-3:-2][0]])
            pair_predict.append([learn_num, model.predict(
                train_data[-1:]), train_data[-1:]])  # 4개면 과거 4개를 넣어서 확인
    ...
    print("장기적 관점 : 정확도 = ", round(bet_accuracyl*100, 2),
          '%', ' 오차 = ', round(bet_lossl*100, 2), '%')
    ...
    #    print("price_one : " ,price_one)
        ...
        # , return_sequences=True))
        model.add(keras.layers.LSTM(
            5, input_shape=(learn_num, 1), dropout=0.1))
        #model.add(keras.layers.LSTM(15, dropout=0.3))
        model.add(keras.layers.Dense(1, activation='relu'))
        model.compile(optimizer='adam', loss='mse', metrics='accuracy')
        print(learn_num, '번째 모델')
...
# 가격 예측모델
    ...
    model = keras.Sequential()
    # , return_sequences=True))
    model.add(keras.layers.LSTM(5, input_shape=(learn_num, 1), dropout=0.1))
    ...
    print("단기적 관점 : 증감의 정확도 = ", round(bet_accuracy*100, 2),
          '%', ' 오차 = ', round(bet_loss*100, 2), '%')
    print('단기적 관점 : 가격예측 오차 = ', round(max(
        history.history['loss'][-3:-2][0], history.history['val_loss'][-3:-2][0])*100, 2), '%')
    # *(max(price_lst-min(price_lst) ) + min(price_lst))*std + mean )
    print('단기적 관점 : 예측가 = ', pval[-1:])
    ...
    for i in range(18, 22):
        for j in range(1, 13):
            ...
            j_z = str(j).zfill(2)
            date = '20{0}{1}'.format(i_z, j_z)
            cnt = final_tst.query('시군구 == "{}"'.format(juso)).query(
                '계약년월=='+date).query(pyengsu[pyeng]).loc[:, '평단가'].count()
